...
export class AppPreBootstrap {
  static run(appRootUrl: string, callback: () => void): void {
    AppConsts.appBaseHref = environment.appBaseUrl;
    AppConsts.remoteServiceBaseUrl = environment.remoteServiceBaseUrl;
    AppConsts.localeMappings = environment.localeMappings;
  ...
  private static getApplicationConfig(appRootUrl: string, callback: () => void) {
    return abp
      ...
      .done((result: { appBaseUrl: string; remoteServiceBaseUrl: string; localeMappings: any }) => {
  ...
  private static getUserConfiguration(callback: () => void): JQueryPromise<any> {
    return abp
      ...
      .done((result: { clock: { provider: string } }) => {
